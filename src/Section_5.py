...
def getBlocks(path_name, col, row, bloc_size):
    """
    Extracts a subblock from a square matrix stored in a .npy file.

    Parameters:
        path_name (str): Path to the .npy file containing the matrix.
        col (int): Column index of the subblock to extract.
        row (int): Row index of the subblock to extract.
        bloc_size (int): Size of the subblock (bloc_size x bloc_size).

    Returns:
        numpy.ndarray: Extracted subblock of the matrix.
    
    Raises:
        ValueError: If the matrix is not square or bloc_size is incompatible.
    """
    # Load the matrix
    matrix = np.load(path_name)
    n = matrix.shape[0]  # Dimension of the square matrix
    # Ensure the matrix is square
    if matrix.shape[0] != matrix.shape[1]:
        raise ValueError(f"Matrix is not square. Shape: {matrix.shape}")
    # Ensure bloc_size divides n evenly
    if n % bloc_size != 0:
        raise ValueError(f"bloc_size {bloc_size} does not evenly divide matrix size {n}.")
    # Calculate the indices for slicing
    start_row = row * bloc_size
    end_row = start_row + bloc_size
    start_col = col * bloc_size
    end_col = start_col + bloc_size
    # Ensure the indices are within bounds
    if start_row >= n or start_col >= n:
        raise ValueError("Requested block indices are out of range.")
    # Extract and return the subblock
...
def parallelNyst(path_name,
    ...
    '''
    A_ij = getBlocks(path_name=path_name, row=row, col=col, bloc_size=bloc_size)


    start_time = time.time() # TODO: set timer after this point.
    ...
    if col == 0:  # Work only with processes in the first column
        ...
        if rank == 0:
            ...
            print(f"Maximum running time across all ranks: {max_time:.6f} seconds")




def test1(path_name = '../Matrices/A_1_n2048_polyDecayMatrix.npy',
          rank_k = 128,  # Target rank for low-rank approximation
          good_conditionned = True,
          initial_sketch_dim = 25,  # Starting sketch dimension
          step = 25, # Step size
          max_sketch_dim = 125,  # Maximum sketch dimension (not included in loop)
          method = 'gaussian',  # Method to use for the sketch ('gaussian' or 'fjlt')
          output_file = '../results_A_1_n2048_polynomialMatrix.csv'  # Name of the .csv file to store the results
          ):
    ...
    with open(output_file, mode='w', newline='') as file:
        ...
        csv_writer.writerow(['sketch_size', 'rank_k', 'method', 'matrix', 'total_time'])

        # Loop over sketch sizes
        for sketch_size in range(initial_sketch_dim, max_sketch_dim, step):
            # Call the parallelNyst function with current sketch_size
            total_time = parallelNyst(
                path_name=path_name,
                rank_k=rank_k,
                good_conditionned=good_conditionned,
                sketch_size=sketch_size,
                method=method
            )
            print(f"Sketch size: {sketch_size}, Total time: {total_time:.4f}")

            # Write the result to the CSV file
            csv_writer.writerow([sketch_size, rank_k, method, path_name, total_time])
...
if __name__ == "__main__":
    ...
    '''
    test1(path_name = '../Matrices/A_1_n2048_polyDecayMatrix.npy',
          rank_k = 128,  # Target rank for low-rank approximation
          good_conditionned = True,
          initial_sketch_dim = 25,  # Starting sketch dimension
          step = 25,  # Step size
    ...
    pathA1 = 'Matrices/A_1_n2048_polyDecayMatrix.npy'
    pathA2 = 'Matrices/A_2_n2048_expDecayMatrix.npy'
    pathA3 = 'Matrices/A_3_dataset_2_mnist_n2048_c100.npy'
    pathA4 = 'Matrices/A_3_dataset_2_mnist_n2048_c10.npy'

    
    parallelNyst(path_name=pathA4,
                    rank_k = 100, # Target rank for low-rank approximation
                    good_conditionned = True,
                    sketch_size = 100,  # Sketch size (number of columns in the random sketch matrices)
                    method = 'fjlt', # or 'fjlt',
                    comm=comm, rank=rank, size=size
                    )
